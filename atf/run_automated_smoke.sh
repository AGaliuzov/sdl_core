...
#    REPORTS_WEB - sccess to reports by web ( will be insersed in html report)

EXECUTE_DIR=${EXECUTE_DIR:-`pwd`}
SDL_BIN_DIR=${SDL_BIN_DIR:-`readlink -f $EXECUTE_DIR/../bin`}
if [ -z "$1" ]; then
  echo "Please pass path to atf build dir as first parameter";
  exit 1;
fi
ATF_BIN_DIR=$1;
REPORTS_DIR=${REPORTS_DIR:-$EXECUTE_DIR/reports};
REPORTS_WEB=${REPORTS_WEB:-$REPORTS_DIR};
